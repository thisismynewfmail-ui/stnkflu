...
from .common import cache_root, data_root, digest, graph_path, save_json
...
def library_path():
    return (cache_root() / "physiology-v6") / (
        "libmemory.dylib" if sys.platform == "darwin" else "libmemory.so"
    )
MODEL = "flylab-compartment-v1"
...
PARAMETERS = {
    **RULE_PARAMETERS,
    ...
    "interpretation": "Candidate KC adaptation/rest plus a baseline-centered anti-Hebbian rate rule extended to the selected mushroom-body compartments. No fitted DAN/MBON background current; lamina bias is a display proxy. Gain, trace constants, compartment count and transfer to this graph remain unvalidated assumptions.",
}
...
class MemoryBrain(NativeBrain):
    def __init__(
        ...
        *,
        eta=0.001,
        circuit=None,
        compartments="alpha1_gamma1pedc",
        ...
        path = graph_path() if path is None else Path(path)
        super().__init__(path)
        ...
        self.circuit = (
            identify(self, compartments) if circuit is None else circuit
        )
        # The in-kernel LTD path is disabled at every call site below
        # (learning_enabled=0), so the kernel never dereferences dan_gain.
        # The plasticity that does run lives in rule.advance, in Python.
        self._unused_kernel_gain = np.zeros(1, dtype=np.float32)
        ...
        if modulation_mask is None:
            ...
            neurons = (
                feather.read_table(data_root() / "normalized/neurons.feather")
                .to_pandas()
                .set_index("source_id")
                .loc[self.ids]
            )
        ...
        *,
        ...
        self.advance(
            ...
            *[
            ...
            self._unused_kernel_gain.ctypes.data,
            self.eta,
    ...
    def configuration_signature(self):
        ...
        return {
            ...
            **{
            ...
            "compartments": self.circuit["preset"],
            "gain": self.circuit["gain"].digest(),
            **{
                k: digest(self.circuit[k]) for k in ["pre", "kc_mask", "dan_index"]
            },
        }
