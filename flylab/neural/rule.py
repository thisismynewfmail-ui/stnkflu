...
from .circuit import project_gain
...
def advance(
    ...
    if frozen:
        return
    # gain may be a dense (DAN x edge) matrix or the compact CompartmentGain;
    # project_gain computes the same gain.T @ vector either way.
    drive = (
        eta
        * (
            kc_hz * project_gain(gain, dmid)
            - project_gain(gain, dan_hz) * kmid
        )
        if learning
